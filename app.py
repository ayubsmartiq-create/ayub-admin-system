...
# =================================================        
# كود نظام إدارة المخزن وطباعة الوصل - مكتبة أيوب الذكية
# =================================================

st.write("---")
st.header("📦 إدارة المخزن وكاشير مكتبة أيوب")

# وظيفة تحميل بيانات المخزن
def load_stock_final():
    if not os.path.exists("data_stock.csv"):
        pd.DataFrame(columns=["المنتج", "الكمية", "السعر"]).to_csv("data_stock.csv", index=False, encoding='utf-8-sig')
    return pd.read_csv("data_stock.csv")

df_stock = load_stock_final()

# تبويبات النظام
tab_sale, tab_inventory, tab_settings = st.tabs(["🛒 نقطة البيع (وصل)", "📊 جرد المخزن", "➕ إضافة منتجات"])

with tab_settings:
    st.subheader("إضافة بضاعة جديدة")
    with st.form("add_new_item", clear_on_submit=True):
        p_name = st.text_input("اسم الكتاب أو القرطاسية")
        p_qty = st.number_input("الكمية المتوفرة", min_value=0, step=1)
        p_price = st.number_input("سعر البيع (دينار)", min_value=0, step=250)
        if st.form_submit_button("إضافة للمخزن ✅"):
            if p_name:
                new_data = pd.DataFrame([[p_name, p_qty, p_price]], columns=["المنتج", "الكمية", "السعر"])
...
with tab_inventory:
    ...
    if not df_stock.empty:
        # تلوين المخزن المنخفض
        def color_stock(val):
            color = 'red' if val < 5 else 'white'
            return f'color: {color}'
        st.dataframe(df_stock.style.applymap(color_stock, subset=['الكمية']), use_container_width=True)
    else:
        st.info("المخزن فارغ حالياً.")

with tab_sale:
    st.subheader("🛒 كاشير المبيعات")
    if not df_stock.empty:
        # محرك البحث الذكي
        selection = st.selectbox("ابحث عن المادة:", df_stock["المنتج"], index=None, placeholder="اكتب اسم المنتج...")
        
        if selection:
            item_info = df_stock[df_stock["المنتج"] == selection].iloc[0]
            st.write(f"💰 السعر: {int(item_info['السعر']):,} د.ع | 📦 المتبقي: {item_info['الكمية']}")
            
            with st.form("sale_final_form"):
                q_sold = st.number_input("الكمية", min_value=1, max_value=int(item_info['الكمية']), value=1)
                if st.form_submit_button("إتمام البيع وإصدار الوصل 🧾"):
                    # 1. خصم من المخزن
                    df_stock.loc[df_stock["المنتج"] == selection, "الكمية"] -= q_sold
                    df_stock.to_csv("data_stock.csv", index=False, encoding='utf-8-sig')
                    
                    # 2. حساب المجموع وتسجيل المالية
                    total_amount = item_info['السعر'] * q_sold
                    exact_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    
                    # حفظ بيانات الوصل في الجلسة (Session)
                    st.session_state['receipt'] = {
                        'name': "مكتبة أيوب الذكية",
                        'time': exact_time,
                        'item': selection,
                        'qty': q_sold,
                        'price': item_info['السعر'],
                        'total': total_amount
                    }
                    
                    # تسجيل في ملف المالية العام
                    df_fin_log = pd.read_csv("data_finance.csv")
                    new_log = pd.DataFrame([[exact_time.split()[0], f"بيع: {selection}", "وارد", total_amount]], columns=FIN_COLS)
                    pd.concat([df_fin_log, new_log], ignore_index=True).to_csv("data_finance.csv", index=False, encoding='utf-8-sig')
                    
                    st.success("تمت العملية بنجاح!")
                    st.rerun()
    else:
        st.warning("يرجى إضافة منتجات في المخزن أولاً.")

    # --- عرض وتصميم الوصل المطبوع ---
    if 'receipt' in st.session_state:
        r = st.session_state['receipt']
        receipt_html = f"""
        <div style="background: white; color: black; padding: 20px; border: 2px solid #000; width: 260px; margin: auto; font-family: 'Cairo', sans-serif; direction: rtl; text-align: center;">
            <h2 style="margin:0;">{r['name']}</h2>
            <p style="font-size:12px; margin:5px 0;">قسم المبيعات الذكي</p>
            <hr style="border: 1px dashed black;">
            <p style="font-size:11px; text-align:right;"><b>الوقت:</b> {r['time']}</p>
            <table style="width:100%; font-size:12px; text-align:right;">
                <tr> <th>المادة</th> <th>العدد</th> <th>السعر</th> </tr>
                <tr> <td>{r['item']}</td> <td>{r['qty']}</td> <td>{int(r['price']):,}</td> </tr>
            </table>
            <hr style="border: 1px dashed black;">
            <h3 style="margin:10px 0;">الإجمالي: {int(r['total']):,} د.ع</h3>
            <p style="font-size:10px;">نتمنى لكم يوماً سعيداً!</p>
            <button onclick="window.print()" style="width:100%; padding:10px; background:black; color:white; border-radius:5px; cursor:pointer;">🖨️ طباعة الوصل</button>
        </div>
        """
        st.write("---")
        st.markdown("### 📄 آخر وصل مبيعات:")
